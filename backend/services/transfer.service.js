    ...
    try {
        const result = await pool.query(
            `SELECT 
                ...
                tr.notes,
                us.ipfs_cid_encrypted,
                us.commitment_hash,
                us.salt_encrypted
             FROM transfer_requests tr
             LEFT JOIN user_secrets us ON tr.parcel_id = us.parcel_id 
                AND tr.from_address = us.owner_address
        ...
        res.json({
            ...
            transfers: result.rows.map(row => ({
                ...
                notes: row.notes,
                ipfs_cid_encrypted: row.ipfs_cid_encrypted,
...
export const requestTransfer = async (req, res) => {
    try {
        ...
        // Validate inputs
        if (!parcelId || !toAddress) {
            return res.status(HTTP_STATUS.BAD_REQUEST).json({
                success: false,
                message: 'Parcel ID and recipient address are required'
            });
        }

        const validatedToAddress = validateAddress(toAddress);

        // Check if parcel exists and belongs to user in user_secrets
        const parcelResult = await pool.query(
            'SELECT owner_address, parcel_id FROM user_secrets WHERE parcel_id = $1',
            [parcelId]
        );
...
export const approveTransfer = async (req, res) => {
    try {
        ...
        try {
            ...
            // Update user_secrets table - change owner
            await pool.query(
                'UPDATE user_secrets SET owner_address = $1 WHERE parcel_id = $2',
